...
本项目在深度诊断页面（DeepDiagnosis）中集成了基于 **ONNX Runtime** 的 AI 推理引擎，通过并行部署多个 ONNX 模型进行集成推理，实现对轴承故障状态的智能分类与置信度评估。模型推理结果以柱状图形式实时展示在诊断界面，辅助运维人员进行设备状态判断。
